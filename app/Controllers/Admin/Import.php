    ...
    public function tramqcb230623()
    {
        ...
        for ($k = 0; $k < $count_sheet; $k++) {
            ...
            foreach ($data as $row) {
                ...
                $code_analysis = null;
                if (is_numeric($row[6])) {
                    $date_manufacture = date("Y-m-d", \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($row[6]));
                } else {
                    $date_manufacture = NULL;
                }
                if (is_numeric($row[7])) {
                    $date_storage = date("Y-m-d", \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($row[7]));
                } else {
                    $date_storage = NULL;
                }
                // $time_text = $row[15];
                ...
                if (is_numeric($row[10])) {
                    $date_theory = date("Y-m-d", \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($row[10]));
                } else {
                    $date_theory = NULL;
                }
                ...
                $type_id = null;
                $time_text = "";
                if (str_contains($env_name, 'Dài hạn (ASEAN)')) {
                    $type_id = 3;
                    $time_text = trim(str_replace('Dài hạn (ASEAN)', "", $env_name));
                } elseif (str_contains($env_name, 'Dài hạn (EU)')) {
                    $type_id = 4;
                    $time_text = trim(str_replace('Dài hạn (EU)', "", $env_name));
                } elseif (str_contains($env_name, 'Lão hóa')) {
                    $type_id = 1;
                    $time_text = trim(str_replace('Lão hóa', "", $env_name));
                }
                echo $env_name . " <br>";
                echo $type_id . " <br>";
                echo $time_text . " <br>";
